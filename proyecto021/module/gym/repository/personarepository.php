...
class PersonaRepository extends Conn {
    ...
    public function getAll() {
        ...
        $sql .= "(select count(idpersonaua) from personaua where idpersona= p.idpersona) as cantidadhijos ";
        $sql .= "FROM persona p ";
    ...
    public function add(Persona $zulma) {
        $sql = "INSERT into persona(documento,nombre,apellidos,genero,fechanacimiento)";
        $sql .= "values (?,?,?,?,?)";
        $resourse = $this->_conn->prepare($sql);
